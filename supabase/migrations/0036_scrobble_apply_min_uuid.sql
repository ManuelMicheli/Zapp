-- Corregge `scrobble_apply` (migration 0035): `min(user_id)` su una colonna
-- uuid non esiste in Postgres, quindi la funzione sollevava
-- `42883 function min(uuid) does not exist` a ogni chiamata. Nessun evento e'
-- mai stato applicato: l'eccezione annulla la transazione, quindi neppure
-- `devices.last_seen_at` restava scritto e da fuori sembrava che le richieste
-- non arrivassero. Il resto della funzione e' identico a 0035.
