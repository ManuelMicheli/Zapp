...
- `src/lib/watch/patch.ts` is the sole source of the state patches ("want"/"watching"/"watched"): both `actions.ts` (Server Action, session) and `core.ts` build the row to write from `entryPatch()`, so the two never diverge on what each status means.
- `src/lib/watch/core.ts` (`applyWatch`) is the write path with a verified `userId` instead of a session, for the device-token routes (App Intents, fase 4: see mobile.md).
