...
export interface DisneyRawMedia {
  titleText: string | null;
  detailText: string | null;
}

/** Parser Disney+ per le stringhe osservate nelle fixture reali.
 * Numeri esposti dal provider: il matching TMDB deve verificarli separatamente.
 */
export function parseDisneyMedia(raw: DisneyRawMedia): ParsedMedia | null {
  if (
    ...
    return null;
  raw = { ...raw, titleText: raw.titleText.replace(/\u2122/g, "").trim() };
