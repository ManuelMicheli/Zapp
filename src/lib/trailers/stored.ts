 ...
 * (riga vecchia senza `lang` o solo con `keys`, JSON corrotto), così il chiamante
 * ricalcola. È anche il modo in cui un cambio di forma invalida il cache da solo.
 */
export function parseTrailers(value: Json): Trailer[] | null {
  ...
  for (const item of value) {
    ...
    out.push({ key, frame: { x, y, w, h }, lang });
  }
  return out;
}
