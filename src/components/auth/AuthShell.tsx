...
/**
 * Guscio comune di login, signup e onboarding: muro di locandine + gradiente +
 * bagliore viola sopra un foglio ancorato in basso, in stile mockup mobile.
 ...
 */
export function AuthShell({ posters, children }: Props) {
  return (
    <div className="relative mx-auto flex min-h-dvh w-full max-w-[480px] flex-col overflow-hidden bg-bg lg:mx-0 lg:grid lg:max-w-none lg:grid-cols-[3fr_minmax(380px,1fr)]">
      <div className="absolute inset-0 overflow-hidden lg:relative lg:inset-auto lg:col-start-1 lg:h-full">
        {/* Mobile: muro alto quanto lo schermo, continua a scorrere dietro il foglio in vetro */}
        <PosterWall posters={posters} height={960} className="lg:hidden" />
        ...
        />
        {/* Mobile: velo che si carica verso il titolo ma lascia trasparire il muro sotto il foglio */}
        <div
          className="absolute inset-0 lg:hidden"
          style={{
            background:
              "linear-gradient(180deg,rgba(0,0,0,.5) 0%,rgba(0,0,0,.12) 20%,rgba(0,0,0,.4) 42%,rgba(0,0,0,.72) 60%,rgba(0,0,0,.78) 100%)",
          }}
        />
