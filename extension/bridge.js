(() => {
  ...
  const runtimeListener = (msg, _sender, sendResponse) => {
  ...
  const pageListener = (e) => {
    ...
    if (!e.data || e.data.canale !== "zapp-capture" || !e.data.dati || typeof e.data.dati.url !== "string") return;
    ...
    try {
      ...
      /* contesto invalidato da un aggiornamento: nessun errore nella pagina */
    }
  };
  ...
  // Handshake: recupera anche l'evento iniziale emesso prima che il bridge esistesse.
  window.postMessage({ canale: "zapp-richiesta" }, location.origin);
})();
